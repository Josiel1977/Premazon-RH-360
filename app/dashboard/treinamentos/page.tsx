...
import {
  Activity, Ban, BarChart3, BookOpenCheck, Boxes, Brain, Building2, CalendarDays,
  CheckCircle2, ChevronDown, ClipboardCheck, ClipboardList, Copy, ExternalLink,
  FileSpreadsheet, GraduationCap, Grid3X3, History, Loader2, Mail, MessageCircle,
  Plus, Printer, Save, Search, Target, TrendingUp, Upload, UserRound, Users,
  WalletCards, X,
} from "lucide-react";
import {
...
type View = "dashboard" | "avaliacao" | "perfil" | "matriz" | "lacunas" | "lnt" | "pdi" | "catalogo" | "custos" | "cronograma" | "custos_setor" | "historico" | "indicadores" | "ninebox";
type Preview =
...
type ProfileResultRecord = {
  id: string; convite_id: string; colaborador_id: string; instrumento_versao: string; algoritmo_versao: string;
  percentuais: Record<DiscDimension, number>; dimensoes_predominantes: DiscDimension[];
  dimensao_secundaria: DiscDimension | null; perfil_combinado: boolean; concluido_em: string;
};
...
const workspaceItems: WorkspaceItem<View>[] = [
  { key: "dashboard", label: "1. Dashboard", icon: BarChart3, tone: "info" },
  { key: "avaliacao", label: "2. Avaliação de Desempenho", icon: ClipboardCheck, tone: "success" },
  { key: "perfil", label: "3. Perfil Comportamental", icon: Brain, tone: "accent" },
  { key: "matriz", label: "4. Matriz de Competências", icon: Grid3X3, tone: "info" },
  { key: "lacunas", label: "5. Análise de Lacunas", icon: TrendingUp, tone: "danger" },
  { key: "lnt", label: "6. Necessidades de Treinamento", icon: ClipboardList, tone: "danger" },
  { key: "pdi", label: "7. PDI", icon: Target, tone: "accent" },
  { key: "catalogo", label: "8. Catálogo de Treinamentos", icon: BookOpenCheck, tone: "warning" },
  { key: "custos", label: "9. Custos e Orçamento", icon: WalletCards, tone: "warning" },
  { key: "cronograma", label: "10. Cronograma Anual", icon: CalendarDays, tone: "info" },
  { key: "custos_setor", label: "11. Custos por Setor", icon: Building2, tone: "warning" },
  { key: "historico", label: "12. Histórico de Treinamentos", icon: History, tone: "success" },
  { key: "indicadores", label: "13. Indicadores", icon: Activity, tone: "info" },
  { key: "ninebox", label: "Matriz 9-Box", icon: Boxes, tone: "accent", dividerBefore: true },
];
...
export default function TreinamentoDesenvolvimentoPage() {
  ...
  const [profileInvites, setProfileInvites] = useState<ProfileInviteRecord[]>([]);
  const [profileResults, setProfileResults] = useState<ProfileResultRecord[]>([]);
  ...
  const loadData = useCallback(async () => {
    ...
    if (!profileError) {
      setProfileInvites((profileInviteResult.data ?? []) as ProfileInviteRecord[]);
      setProfileResults((profileResult.data ?? []).map((item) => ({
        ...item,
        percentuais: Object.fromEntries(Object.entries(item.percentuais ?? {}).map(([key, value]) => [key, Number(value)])) as Record<DiscDimension, number>,
      })) as ProfileResultRecord[]);
    }
    setLoading(false);
  }, []);
  ...
  async function createProfileInvitation(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setSaving(true); setMessage(null); setGeneratedProfileLink("");
    const form = new FormData(event.currentTarget);
    const collaboratorId = String(form.get("colaborador_id") ?? "");
    const expirationDate = String(form.get("expira_em") ?? "");
    const expiration = new Date(`${expirationDate}T23:59:59-03:00`);
    if (!collaboratorId || !expirationDate || !Number.isFinite(expiration.getTime())) {
      setMessage({ type: "error", text: "Selecione o colaborador e uma data de validade." }); setSaving(false); return;
    }
    const { data, error } = await supabase.from("td_perfil_convites").insert({
      colaborador_id: collaboratorId,
      instrumento_versao: DISC_INSTRUMENT_VERSION,
      finalidade: "Autoconhecimento e planejamento de desenvolvimento profissional",
      expira_em: expiration.toISOString(),
      status: "pendente",
    }).select("public_token").single();
    if (error) {
      setMessage({ type: "error", text: error.message.includes("td_perfil_convites") ? "Execute a migração 011 antes de gerar o convite." : error.message });
    } else {
      const link = `${window.location.origin}/perfil/${data.public_token}`;
      setGeneratedProfileLink(link);
      setMessage({ type: "success", text: "Link individual criado. Envie-o somente ao colaborador selecionado." });
      await loadData();
    }
    setSaving(false);
  }

  async function revokeProfileInvitation(id: string) {
    setSaving(true); setMessage(null);
    const { error } = await supabase.from("td_perfil_convites").update({ status: "revogado", revogado_em: new Date().toISOString() }).eq("id", id).eq("status", "pendente");
    if (error) setMessage({ type: "error", text: error.message });
    else { setMessage({ type: "success", text: "Convite revogado. O link deixou de aceitar respostas." }); await loadData(); }
    setSaving(false);
  }
  ...
  return (
    <ModuleWorkspace eyebrow="Premazon RH 360 · Programa estratégico" title="Treinamento & Desenvolvimento" description="Da avaliação ao resultado: competências, lacunas, LNT, PDI, catálogo, orçamento, execução e indicadores em uma jornada auditável." icon={GraduationCap} items={workspaceItems} active={view} onChange={setView} accent="from-emerald-950 via-teal-900 to-cyan-800" actions={<><button type="button" onClick={() => setShowImport((value) => !value)} className="flex items-center rounded-xl bg-white/10 px-3 py-2 text-xs font-black ring-1 ring-white/20 hover:bg-white/20"><Upload className="mr-2 h-4 w-4" />Importar novos dados</button><button type="button" onClick={() => { setView("historico"); setShowTrainingForm(true); }} className="flex items-center rounded-xl bg-amber-400 px-3 py-2 text-xs font-black text-slate-950 hover:bg-amber-300"><Plus className="mr-2 h-4 w-4" />Planejar ação</button></>}>
      <div className="space-y-5">
        ...
        {loading ? <div className="flex items-center justify-center rounded-2xl border border-slate-200 bg-white p-16 text-sm text-slate-500"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Carregando programa…</div> : <>
          ...
          {view === "perfil" && <BehavioralProfileView signals={filteredSignals} selectedId={effectiveSignalId} onSelect={setSelectedSignalId} selected={selectedSignal} collaborators={filteredCollaborators} invites={filteredProfileInvites} results={filteredProfileResults} ready={profileReady} generatedLink={generatedProfileLink} saving={saving} onCreateInvitation={createProfileInvitation} onRevokeInvitation={revokeProfileInvitation} />}
...
function EmployeeView({ signals, selectedId, onSelect, selected, radar, gaps, needs }: { signals: SignalRecord[]; selectedId: string; onSelect: (id: string) => void; selected: SignalRecord | null; radar: { name: string; nota: number | null }[]; gaps: [string, { nota: number; evidencia?: string | null }][]; needs: NeedRecord[] }) {
  ...
  return <div className="space-y-5"><SectionTitle title="Avaliação de Desempenho" description="Resultado individual, evidências, pontos fortes, lacunas e necessidades em uma visão auditável." />
...
function BehavioralProfileView({ signals, selectedId, onSelect, selected, collaborators, invites, results, ready, generatedLink, saving, onCreateInvitation, onRevokeInvitation }: {
  ...
  const scores: { name: string; nota: number }[] = selected ? TD_COMPETENCIES
    ...
    .flatMap((item) => {
      const score = selected.competencias[item.key]?.nota;
      return score == null ? [] : [{ name: String(item.label), nota: Number(score) }];
    }) : [];
  const strongest = [...scores].sort((a, b) => b.nota - a.nota).slice(0, 3);
  ...
  const resultLabel = (result: ProfileResultRecord) => result.dimensoes_predominantes.length === 4
    ...
    : result.dimensoes_predominantes.map((dimension) => DISC_PROFILES[dimension].name).join(" + ");
  const linkFor = (token: string) => `${window.location.origin}/perfil/${token}`;
  const copyInvitation = async (token: string) => navigator.clipboard.writeText(linkFor(token));
  ...
  return <div className="space-y-5"><SectionTitle title="Perfil Comportamental" description="Convide o colaborador para responder o questionário de autopercepção e use o resultado como apoio ao diálogo de desenvolvimento." />
    ...
    {!signals.length || !selected ? <WorkspaceEmpty icon={Brain} title="Avaliação de desempenho não importada" description="Esta leitura complementar será exibida quando existirem competências comportamentais avaliadas." /> : <>
      <ProgramPanel title="Selecionar colaborador"><div className="p-5"><select value={selectedId} onChange={(event) => onSelect(event.target.value)} className={inputClass}>{signals.map((item) => <option key={item.id} value={item.id}>{item.colaborador_nome_importado} · {item.setor_importado}</option>)}</select></div></ProgramPanel>
      <div className="grid gap-5 xl:grid-cols-[1.2fr_.8fr]"><ProgramPanel title={selected.colaborador_nome_importado} description={`${selected.cargo_importado} · ${selected.setor_importado}`}><div className="h-[420px] p-4"><ResponsiveContainer width="100%" height="100%"><RadarChart data={scores}><PolarGrid /><PolarAngleAxis dataKey="name" tick={{ fontSize: 10 }} /><Radar dataKey="nota" stroke="#7c3aed" fill="#8b5cf6" fillOpacity={0.3} /><Tooltip /></RadarChart></ResponsiveContainer></div></ProgramPanel>
        <div className="space-y-5"><ProgramPanel title="Competências mais altas"><div className="space-y-2 p-5">{strongest.map((item) => <div key={item.name} className="flex items-center justify-between rounded-xl bg-emerald-50 px-3 py-3 text-xs"><span className="font-bold text-emerald-900">{item.name}</span><Pill tone="emerald">{item.nota.toFixed(1)}</Pill></div>)}{!strongest.length && <p className="text-xs text-slate-500">Sem notas comportamentais válidas.</p>}</div></ProgramPanel><ProgramPanel title="Competências para desenvolver"><div className="space-y-2 p-5">{development.map((item) => <div key={item.name} className="flex items-center justify-between rounded-xl bg-amber-50 px-3 py-3 text-xs"><span className="font-bold text-amber-900">{item.name}</span><Pill tone="amber">{item.nota.toFixed(1)}</Pill></div>)}{!development.length && <p className="text-xs text-slate-500">Sem notas comportamentais válidas.</p>}</div></ProgramPanel></div>
      </div>
    </>}
  </div>;
}
...
function PdiView({ signals, selectedId, onSelect, selected, gaps, courses, pdis, saving, onSubmit }: { signals: SignalRecord[]; selectedId: string; onSelect: (id: string) => void; selected: SignalRecord | null; gaps: [string, { nota: number }][]; courses: CourseRecord[]; pdis: PdiRecord[]; saving: boolean; onSubmit: (event: React.FormEvent<HTMLFormElement>) => void }) {
  return <div className="space-y-5"><SectionTitle title="Plano de Desenvolvimento Individual (PDI)" description="Plano construído sobre a avaliação real, com ações, resultado esperado, prazo e acompanhamento." />
...
function TypesView({ courses, trainings, summary, onNew }: { courses: CourseRecord[]; trainings: TrainingRecord[]; summary: { name: string; value: number }[]; onNew: () => void }) {
  return <div className="space-y-5"><SectionTitle title="Catálogo de Treinamentos" description="Portfólio corporativo e leitura do histórico por categoria, modalidade e obrigatoriedade." /><div className="grid gap-5 xl:grid-cols-[.8fr_1.2fr]"><ProgramPanel title="Ações por categoria"><MiniBarList items={summary} empty="Planeje treinamentos para formar o histórico." color="bg-violet-600" /></ProgramPanel><ProgramPanel title="Catálogo corporativo" action={<button type="button" onClick={onNew} className="flex items-center rounded-lg bg-primary px-3 py-2 text-xs font-black text-white"><Plus className="mr-1.5 h-4 w-4" />Novo curso</button>}><div className="grid gap-3 p-5 md:grid-cols-2">{courses.map((course) => <article key={course.id} className="rounded-xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-2"><Pill tone={course.obrigatorio ? "red" : "blue"}>{categoryLabels[course.categoria] ?? course.categoria}</Pill><span className="text-xs font-black text-slate-500">{course.carga_horaria}h</span></div><h4 className="mt-3 text-sm font-black text-slate-800">{course.nome}</h4><p className="mt-1 text-xs text-slate-500">{course.modalidade} · {course.validade_meses ? `validade ${course.validade_meses} meses` : "sem validade"}</p></article>)}</div>{!courses.length && <p className="p-8 text-center text-xs text-slate-500">Nenhum curso cadastrado.</p>}</ProgramPanel></div><MetricCard label="Treinamentos registrados" value={trainings.length} detail="Planejados, em andamento e concluídos" icon={GraduationCap} tone="violet" /></div>;
}

function MatrixView({ signals }: { signals: SignalRecord[] }) {
  return <div className="space-y-5"><SectionTitle title="Matriz de Competências" description="Comparação pessoa × competência, com recorte por setor. Células vazias permanecem vazias; o sistema não completa notas." /><ProgramPanel title="Matriz de competências"><div className="overflow-x-auto"><table className="min-w-[1500px] text-left text-[10px]"><thead className="sticky top-0 bg-slate-900 text-white"><tr><th className="px-3 py-3">Colaborador</th><th className="px-3 py-3">Setor</th>{TD_COMPETENCIES.map((item) => <th key={item.key} className="px-2 py-3 text-center">{item.label}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{signals.map((signal) => <tr key={signal.id}><td className="whitespace-nowrap px-3 py-3 font-bold text-slate-800">{signal.colaborador_nome_importado}</td><td className="px-3 py-3 text-slate-500">{signal.setor_importado}</td>{TD_COMPETENCIES.map((item) => { const score = signal.competencias[item.key]?.nota; const style = score == null ? "bg-slate-50 text-slate-300" : score < 6 ? "bg-red-100 text-red-800" : score < 8 ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"; return <td key={item.key} className="px-1 py-2 text-center"><span className={`inline-flex h-7 w-8 items-center justify-center rounded-md font-black ${style}`}>{score ?? "—"}</span></td>; })}</tr>)}</tbody></table>{!signals.length && <p className="p-8 text-center text-xs text-slate-500">Nenhuma avaliação disponível.</p>}</div></ProgramPanel></div>;
}

function GapAnalysisView({ signals, gaps, sectorNeeds }: { signals: SignalRecord[]; gaps: { name: string; value: number }[]; sectorNeeds: { name: string; value: number }[] }) {
  const people = signals.map((signal) => {
    const personGaps = Object.entries(signal.competencias)
      .filter(([, value]) => Number(value.nota) < 7)
      .sort((a, b) => Number(a[1].nota) - Number(b[1].nota));
    return { signal, gaps: personGaps };
  }).sort((a, b) => b.gaps.length - a.gaps.length);
  const totalGaps = people.reduce((sum, item) => sum + item.gaps.length, 0);
  const peopleWithGaps = people.filter((item) => item.gaps.length > 0).length;

  return <div className="space-y-5"><SectionTitle title="Análise de Lacunas" description="Diferenças identificadas nas competências avaliadas, usando nota abaixo de 7 como critério transparente de atenção." />
    <div className="grid gap-4 sm:grid-cols-3"><MetricCard label="Pessoas no recorte" value={signals.length} detail="Avaliações válidas" icon={Users} tone="blue" /><MetricCard label="Pessoas com lacunas" value={peopleWithGaps} detail="Ao menos uma competência abaixo de 7" icon={TrendingUp} tone="red" /><MetricCard label="Ocorrências de lacuna" value={totalGaps} detail="Soma das competências abaixo de 7" icon={ClipboardList} tone="amber" /></div>
    <div className="grid gap-5 xl:grid-cols-2"><ProgramPanel title="Lacunas mais frequentes" description="Quantidade de pessoas por competência"><MiniBarList items={gaps} color="bg-red-500" empty="Nenhuma lacuna encontrada no recorte atual." /></ProgramPanel><ProgramPanel title="LNT aberta por setor" description="Necessidades ainda não atendidas"><MiniBarList items={sectorNeeds} color="bg-teal-600" empty="Nenhuma necessidade aberta no recorte atual." /></ProgramPanel></div>
    <ProgramPanel title="Detalhamento por pessoa" description="Prioriza quem possui mais competências abaixo do critério"><div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left text-xs"><thead className="bg-slate-50 uppercase text-slate-500"><tr><th className="px-5 py-3">Colaborador</th><th className="px-5 py-3">Setor</th><th className="px-5 py-3">Média</th><th className="px-5 py-3">Lacunas</th><th className="px-5 py-3">Prioridades observadas</th></tr></thead><tbody className="divide-y divide-slate-100">{people.map(({ signal, gaps: personGaps }) => <tr key={signal.id}><td className="px-5 py-4 font-bold text-slate-800">{signal.colaborador_nome_importado}</td><td className="px-5 py-4 text-slate-600">{signal.setor_importado}</td><td className="px-5 py-4 font-black text-slate-700">{signal.media_geral.toFixed(1)}</td><td className="px-5 py-4"><Pill tone={personGaps.length ? "red" : "emerald"}>{personGaps.length}</Pill></td><td className="px-5 py-4 text-slate-600">{personGaps.slice(0, 3).map(([key]) => competenceLabel(key)).join(", ") || "Sem competência abaixo de 7"}</td></tr>)}</tbody></table>{!people.length && <p className="p-8 text-center text-xs text-slate-500">Importe a avaliação de desempenho para analisar lacunas.</p>}</div></ProgramPanel>
  </div>;
}

function LntView({ needs, onUpdate }: { needs: NeedRecord[]; onUpdate: (id: string, field: "prioridade" | "status", value: string) => void }) {
  return <div className="space-y-5"><SectionTitle title="Necessidades de Treinamento (LNT)" description="Base consolidada, priorização e transformação da necessidade em ação de desenvolvimento." /><ProgramPanel title="Levantamento de necessidades" description={`${needs.length} registro(s) no recorte atual`}><div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-xs"><thead className="bg-slate-50 uppercase text-slate-500"><tr><th className="px-4 py-3">Colaborador</th><th className="px-4 py-3">Setor / Cargo</th><th className="px-4 py-3">Necessidades</th><th className="px-4 py-3">Sugestão</th><th className="px-4 py-3">Prioridade</th><th className="px-4 py-3">Status</th></tr></thead><tbody className="divide-y divide-slate-100">{needs.map((item) => <tr key={item.id}><td className="px-4 py-4"><p className="font-bold text-slate-800">{item.colaborador_nome_importado}</p><p className="mt-1 text-[10px] text-slate-400">Gestor: {item.gestor_importado}</p></td><td className="px-4 py-4 text-slate-600">{item.setor_importado}<br />{item.cargo_importado}</td><td className="max-w-sm px-4 py-4 text-slate-600">{[...item.necessidades_tecnicas, ...item.temas_comportamentais].join(", ") || "Não informada"}</td><td className="px-4 py-4 text-slate-600">{item.treinamento_sugerido || "—"}</td><td className="px-4 py-4"><select value={item.prioridade} onChange={(event) => void onUpdate(item.id, "prioridade", event.target.value)} className="rounded-lg border border-slate-200 p-2"><option value="baixa">Baixa</option><option value="media">Média</option><option value="alta">Alta</option><option value="critica">Crítica</option></select></td><td className="px-4 py-4"><select value={item.status} onChange={(event) => void onUpdate(item.id, "status", event.target.value)} className="rounded-lg border border-slate-200 p-2"><option value="identificada">Identificada</option><option value="priorizada">Priorizada</option><option value="planejada">Planejada</option><option value="atendida">Atendida</option><option value="cancelada">Cancelada</option></select></td></tr>)}</tbody></table>{!needs.length && <p className="p-8 text-center text-xs text-slate-500">Nenhuma necessidade encontrada.</p>}</div></ProgramPanel></div>;
}
...
function ManagementView({ trainings, participations, onNew }: { trainings: TrainingRecord[]; participations: ParticipationRecord[]; onNew: () => void }) {
  return <div className="space-y-5"><SectionTitle title="Histórico de Treinamentos" description="Plano e histórico de execução, público, investimento e participação em uma base operacional única." /><ProgramPanel title="Treinamentos registrados" action={<button type="button" onClick={onNew} className="flex items-center rounded-lg bg-primary px-3 py-2 text-xs font-black text-white"><Plus className="mr-1.5 h-4 w-4" />Planejar</button>}><div className="grid gap-4 p-5 lg:grid-cols-2">{trainings.map((item) => { const people = participations.filter((person) => person.treinamento_id === item.id); return <article key={item.id} className="rounded-xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><Pill tone={item.status === "concluido" ? "emerald" : item.status === "cancelado" ? "red" : "blue"}>{statusLabels[item.status] ?? item.status}</Pill><h4 className="mt-3 text-sm font-black text-slate-800">{item.titulo}</h4><p className="mt-1 text-xs text-slate-500">{formatDate(item.data_inicio)} · {item.carga_horaria}h · {item.modalidade}</p></div><span className="text-xs font-black text-slate-600">{money.format(item.custo_real ?? item.custo_planejado ?? 0)}</span></div><div className="mt-4 flex gap-4 border-t border-slate-100 pt-3 text-[10px] font-bold text-slate-500"><span>{people.length} participante(s)</span><span>{item.publico_alvo || "Público não informado"}</span></div></article>; })}</div>{!trainings.length && <p className="p-8 text-center text-xs text-slate-500">Nenhum treinamento registrado.</p>}</ProgramPanel></div>;
}
...
function CostsView({ planned, real, benefit, onBenefit, roi, trainings }: { planned: number; real: number; benefit: string; onBenefit: (value: string) => void; roi: number | null; trainings: TrainingRecord[] }) {
  return <div className="space-y-5"><SectionTitle title="Custos e Orçamento" description="Planejado, realizado e cálculo transparente de retorno. O ROI só é exibido após informar um benefício mensurado." /><div className="grid gap-4 sm:grid-cols-3"><MetricCard label="Custo planejado" value={money.format(planned)} icon={WalletCards} tone="amber" /><MetricCard label="Custo realizado" value={money.format(real)} icon={CheckCircle2} tone="emerald" /><MetricCard label="ROI calculado" value={roi == null ? "Pendente" : `${roi.toFixed(1)}%`} detail={roi == null ? "Informe o benefício mensurado" : "(benefício − investimento) ÷ investimento"} icon={BarChart3} tone={roi != null && roi >= 0 ? "emerald" : "red"} /></div><ProgramPanel title="Mensuração do benefício" description="Ex.: redução de retrabalho, acidentes, horas extras ou perdas, validada pela área responsável"><div className="grid gap-4 p-5 md:grid-cols-2"><label className="text-xs font-bold text-slate-700">Benefício financeiro mensurado (R$)<input type="number" min="0" step="0.01" value={benefit} onChange={(event) => onBenefit(event.target.value)} className={inputClass} /></label><div className="rounded-xl bg-blue-50 p-4 text-xs leading-5 text-blue-800"><strong>Critério:</strong> o sistema não presume retorno. Registre somente um benefício que possa ser demonstrado por indicador e período.</div></div></ProgramPanel><ProgramPanel title="Custos por ação"><MiniBarList items={trainings.filter((item) => (item.custo_real ?? item.custo_planejado) != null).map((item) => ({ name: item.titulo, value: item.custo_real ?? item.custo_planejado ?? 0 })).sort((a, b) => b.value - a.value)} valueLabel={(value) => money.format(value)} color="bg-amber-500" empty="Nenhum custo informado no plano." /></ProgramPanel></div>;
}

function SectorCostsView({ trainings }: { trainings: TrainingRecord[] }) {
  const grouped = new Map<string, number>();
  trainings.forEach((item) => {
    const allocation = item.publico_alvo?.trim() || "Sem alocação informada";
    grouped.set(allocation, (grouped.get(allocation) ?? 0) + (item.custo_real ?? item.custo_planejado ?? 0));
  });
  const items = [...grouped].map(([name, value]) => ({ name, value })).filter((item) => item.name !== "Sem alocação informada" && item.value > 0).sort((a, b) => b.value - a.value);
  const unallocated = grouped.get("Sem alocação informada") ?? 0;
  const total = trainings.reduce((sum, item) => sum + (item.custo_real ?? item.custo_planejado ?? 0), 0);

  return <div className="space-y-5"><SectionTitle title="Custos por Setor" description="Consolidação do custo pelo público-alvo ou setor informado no cadastro de cada treinamento." />
    <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs leading-5 text-amber-900"><strong>Critério de alocação:</strong> a base atual possui o campo “público-alvo”, mas ainda não um vínculo estrutural exclusivo com setores. Por isso, a plataforma mostra a descrição registrada e não distribui valores automaticamente entre áreas.</div>
    <div className="grid gap-4 sm:grid-cols-3"><MetricCard label="Custo consolidado" value={money.format(total)} detail="Planejado ou realizado por ação" icon={WalletCards} tone="amber" /><MetricCard label="Alocações informadas" value={items.length} detail="Descrições distintas com custo" icon={Building2} tone="blue" /><MetricCard label="Sem alocação" value={money.format(unallocated)} detail="Requer classificação pelo RH" icon={ClipboardList} tone={unallocated > 0 ? "red" : "emerald"} /></div>
    <ProgramPanel title="Distribuição por setor / público informado" description="O custo realizado substitui o planejado quando estiver disponível"><MiniBarList items={items} valueLabel={(value) => money.format(value)} color="bg-cyan-600" empty="Informe o setor ou público-alvo e o custo nos treinamentos para formar esta visão." /></ProgramPanel>
  </div>;
}
...
function IndicatorsView({ signals, needs, trainings, participations, pdis, distribution }: { signals: SignalRecord[]; needs: NeedRecord[]; trainings: TrainingRecord[]; participations: ParticipationRecord[]; pdis: PdiRecord[]; distribution: { name: string; value: number }[] }) {
  const validTrainings = trainings.filter((item) => item.status !== "cancelado");
  const completedTrainings = validTrainings.filter((item) => item.status === "concluido");
  const completionRate = validTrainings.length ? (completedTrainings.length / validTrainings.length) * 100 : null;
  const averagePerformance = signals.length ? signals.reduce((sum, item) => sum + item.media_geral, 0) / signals.length : null;
  const attendanceValues = participations.map((item) => item.frequencia_percentual).filter((value): value is number => value != null);
  ...
  const openNeeds = needs.filter((item) => !["atendida", "cancelada"].includes(item.status)).length;
  const activePdis = pdis.filter((item) => item.status === "ativo").length;
  const today = new Date().toISOString().slice(0, 10);
  const overduePdis = pdis.filter((item) => item.status === "ativo" && item.data_limite && item.data_limite < today).length;
  const totalHours = completedTrainings.reduce((sum, item) => sum + item.carga_horaria, 0);
  const statusSummary = groupCount(trainings, (item) => statusLabels[item.status] ?? item.status);

  return <div className="space-y-5"><SectionTitle title="Indicadores de Treinamento e Desenvolvimento" description="Indicadores calculados somente a partir dos registros atuais, com ausência de dado representada por “—”." />
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Pessoas avaliadas" value={signals.length} detail="Avaliações válidas" icon={Users} tone="blue" /><MetricCard label="Média de desempenho" value={averagePerformance == null ? "—" : averagePerformance.toFixed(1)} detail="Escala de 0 a 10" icon={Target} tone="emerald" /><MetricCard label="LNT em aberto" value={openNeeds} detail="Identificada, priorizada ou planejada" icon={ClipboardList} tone="red" /><MetricCard label="Conclusão do plano" value={completionRate == null ? "—" : `${completionRate.toFixed(1)}%`} detail={`${completedTrainings.length} de ${validTrainings.length} ação(ões)`} icon={CheckCircle2} tone="emerald" /><MetricCard label="Participações" value={participations.length} detail="Registros de participantes" icon={GraduationCap} tone="violet" /><MetricCard label="Frequência média" value={averageAttendance == null ? "—" : `${averageAttendance.toFixed(1)}%`} detail={`${attendanceValues.length} registro(s) com frequência`} icon={Activity} tone="blue" /><MetricCard label="Carga horária concluída" value={`${totalHours.toFixed(1)}h`} detail="Soma das ações concluídas" icon={History} tone="amber" /><MetricCard label="PDIs ativos / vencidos" value={`${activePdis} / ${overduePdis}`} detail="Acompanhamento de prazo" icon={Target} tone={overduePdis ? "red" : "emerald"} /></div>
    <div className="grid gap-5 xl:grid-cols-2"><ProgramPanel title="Status dos treinamentos" description="Distribuição das ações registradas"><MiniBarList items={statusSummary} color="bg-cyan-600" empty="Nenhum treinamento registrado." /></ProgramPanel><ProgramPanel title="Desempenho por faixa" description="Classificação sobre médias válidas"><div className="h-72 p-4"><ResponsiveContainer width="100%" height="100%"><BarChart data={distribution}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="name" tick={{ fontSize: 11 }} /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="value" radius={[8, 8, 0, 0]}>{distribution.map((_, index) => <Cell key={index} fill={colors[index % colors.length]} />)}</Bar></BarChart></ResponsiveContainer></div></ProgramPanel></div>
  </div>;
}
